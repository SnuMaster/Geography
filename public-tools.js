(() => {
  ...
  function renderFeedbackWidget(){
    if(document.getElementById('korgeoFeedbackBtn'))return;
    ...
    const modal=document.createElement('div');modal.id='korgeoFeedbackModal';modal.hidden=true;modal.style.cssText='position:fixed;inset:0;z-index:10000;background:#0007;display:grid;place-items:center;padding:18px';
    ...
    btn.onclick=open;card.querySelector('#korgeoFeedbackClose').onclick=close;modal.onclick=e=>{if(e.target===modal)close();};
    card.querySelector('#korgeoFeedbackSend').onclick=async()=>{const send=card.querySelector('#korgeoFeedbackSend');const status=card.querySelector('#korgeoFeedbackStatus');const text=card.querySelector('#korgeoFeedbackText').value.trim();const category=card.querySelector('#korgeoFeedbackCategory').value;if(text.length<2){status.textContent='내용을 2자 이상 적어줘.';return;}send.disabled=true;status.textContent='보내는 중…';const result=await window.korgeoSubmitFeedback(category,text);if(result?.ok){status.textContent='접수 완료. 고마워!';card.querySelector('#korgeoFeedbackText').value='';setTimeout(close,900);}else if(result?.error==='rate_limited'){status.textContent='제보를 너무 많이 보냈어. 조금 뒤 다시 해줘.';}else status.textContent='전송 실패. 잠시 후 다시 해줘.';send.disabled=false;};
  }

  trackVisit();
  const boot=()=>{loadPublicState();renderFeedbackWidget();};

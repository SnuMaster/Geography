(() => {
  ...
  function boot() {
    ...
    authClient.auth.onAuthStateChange((_event, session) => {
      if (session?.user && accountName) accountName.textContent = displayName(session.user);
    });
    authClient.auth.getSession().then(({ data }) => {
      if (data.session?.user && accountName) accountName.textContent = displayName(data.session.user);
    });
  }

(() => {
  ...
  function boot(){
    ...
    let captchaToken='';let captchaWidgetId=null;let captchaWrap=null;let turnstileReadyPromise=null;
    ...
    function ensureTurnstile(){
      ...
      turnstileReadyPromise=new Promise(resolve=>{
        ...
        if(!script){script=document.createElement('script');script.src='https://challenges.cloudflare.com/turnstile/v0/api.js?render=explicit';script.async=true;script.defer=true;script.dataset.korgeoTurnstile='1';document.head.appendChild(script);}
        script.addEventListener('load',render,{once:true});script.addEventListener('error',()=>resolve(false),{once:true});setTimeout(()=>{if(window.turnstile)render();else resolve(false);},5000);
      });
      return turnstileReadyPromise;
    }
    async function requireCaptcha(){await ensureTurnstile();if(!captchaToken){setStatus('사람인지 확인하는 보안 확인을 먼저 완료해줘.','error');return null;}return captchaToken;}
    function resetCaptcha(){captchaToken='';if(window.turnstile&&captchaWidgetId!==null){try{window.turnstile.reset(captchaWidgetId);}catch{}}}
    ...
    loginBtn.onclick=async()=>{
      const c=credentials();if(c.error)return setStatus(c.error,'error');
      const token=await requireCaptcha();if(!token)return;
      setStatus('로그인 중…');
      try{
        const{data,error}=await authClient.auth.signInWithPassword({email:c.email,password:c.password,options:{captchaToken:token}});
        if(error)return setStatus(/captcha/i.test(error.message||'')?'보안 확인이 만료됐어. 다시 확인해줘.':'로그인 실패 · 아이디 또는 비밀번호를 확인해줘.','error');
    ...
    actualSignupBtn.onclick=async()=>{
      const c=credentials();if(c.error)return setStatus(c.error,'error');
      const token=await requireCaptcha();if(!token)return;
      actualSignupBtn.disabled=true;setStatus('회원가입 확인 중…');
      try{
        await ensurePublicTools();
        const slot=window.korgeoClaimSignupSlot?await window.korgeoClaimSignupSlot():{allowed:true};
        if(!slot.allowed){if(slot.registration_disabled)return setStatus('현재 관리자가 신규 회원가입을 잠시 꺼둔 상태야.','error');const sec=Math.max(1,Number(slot.retry_after||60));return setStatus(`가입 시도가 너무 많아. 약 ${Math.ceil(sec/60)}분 뒤 다시 해줘.`,'error');}
        setStatus('회원가입 중…');
        const{data,error}=await authClient.auth.signUp({email:c.email,password:c.password,options:{data:{username:c.username},captchaToken:token}});
        if(error){const duplicate=/already|registered|exists/i.test(error.message||'');return setStatus(/captcha/i.test(error.message||'')?'보안 확인이 만료됐어. 다시 확인해줘.':duplicate?'이미 사용 중인 아이디야.':'회원가입 실패: '+error.message,'error');}
        if(data.session){if(await enforceAccess(data.user))setStatus('회원가입 완료 · 바로 로그인됐어.','success');}else setStatus('가입은 됐지만 로그인 세션이 없어. Confirm email 설정을 확인해줘.','error');
      }finally{resetCaptcha();setTimeout(()=>{actualSignupBtn.disabled=false;},3000);}
    };

    passwordInput.addEventListener('keydown',event=>{if(event.key==='Enter')loginBtn.click();});
    authClient.auth.onAuthStateChange((_event,session)=>{const user=session?.user||null;if(captchaWrap)captchaWrap.style.display=user?'none':'flex';if(user&&accountName)accountName.textContent=displayName(user);if(user)setTimeout(()=>void enforceAccess(user),0);});
    authClient.auth.getSession().then(({data})=>{const user=data.session?.user;if(captchaWrap)captchaWrap.style.display=user?'none':'flex';if(user&&accountName)accountName.textContent=displayName(user);if(user)void enforceAccess(user);});
  }

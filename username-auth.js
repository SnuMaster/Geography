(() => {
  ...
  function addPasswordToggle(input) {
    if (!input || document.getElementById('korgeoPasswordToggleMain')) return;
    ...
    button.id = 'korgeoPasswordToggleMain';
    button.type = 'button';
  ...
  function clearCaptchaError() {
    if (/^Cloudflare 보안 확인 오류/.test(account.textContent || '')) {
      account.textContent = '보안 확인 완료 · 아이디와 비밀번호를 입력해줘.';
    }
  }
        ...
        if (captchaWidgetId === null) {
          try {
            captchaWidgetId = window.turnstile.render(captchaWrap, {
              ...
              'error-callback': code => {
                captchaToken = '';
                account.textContent = 'Cloudflare 보안 확인 오류 (' + code + ').';
                return true;
              }
            });
          } catch {
  ...
  async function requireCaptcha() {
    ...
    if (!captchaToken) {
      account.textContent = '사람인지 확인하는 보안 확인을 먼저 완료해줘.';
      return null;
    }
    return captchaToken;
  }
  ...
  function credentials() {
    ...
    if (!USERNAME_RE.test(username)) return { error: '아이디는 영문 소문자, 숫자, 점(.), 밑줄(_), 하이픈(-)만 써서 3~24자로 만들어줘.' };
  ...
  function renderUsernameSession(user) {
    ...
    if (captchaWrap) captchaWrap.style.display = loggedIn ? 'none' : 'flex';
    const toggle = document.getElementById('korgeoPasswordToggleMain');
    if (passwordInput.parentElement) passwordInput.parentElement.style.display = loggedIn ? 'none' : 'block';
    if (toggle) toggle.hidden = loggedIn;
  ...
  async function enforceAccess(user) {
    if (!user) return true;
    try {
      const { data, error } = await supabaseClient.rpc('get_my_access_state');
      if (error) return true;
      if (data?.suspended) {
        const reason = String(data.reason || '').trim();
        await supabaseClient.auth.signOut();
        account.textContent = '이용정지된 계정이야.' + (reason ? ' 사유: ' + reason : '');
        return false;
      }
    } catch {}
    return true;
  ...
  loginBtn.onclick = async () => {
    ...
    try {
      ...
      if (error) {
        account.textContent = /captcha/i.test(error.message || '')
          ? '보안 확인이 만료됐어. 다시 확인해줘.'
  ...
  signupBtn.onclick = async () => {
    ...
    try {
      ...
      if (!slot.allowed) {
        if (slot.registration_disabled) {
          account.textContent = '현재 관리자가 신규 회원가입을 잠시 꺼둔 상태야.';
          return;
        }
        if (slot.unavailable) {
          account.textContent = '가입 상태를 확인하지 못했어. 잠시 후 다시 시도해줘.';
          return;
        }
        const sec = Math.max(1, Number(slot.retry_after || 60));
        account.textContent = `가입 시도가 너무 많아. 약 ${Math.ceil(sec / 60)}분 뒤 다시 해줘.`;
        return;
      }
      ...
      if (data.session) {
        if (await enforceAccess(data.user)) account.textContent = `회원가입 완료 · ${c.username} 아이디로 바로 로그인됐어.`;
      } else {
        account.textContent = '회원가입은 됐지만 로그인 세션이 만들어지지 않았어. Confirm email 설정을 확인해줘.';
      }
    } catch (error) {
  ...
  supabaseClient.auth.onAuthStateChange((_event, session) => queueMicrotask(() => {
    const user = session?.user || null;
    renderUsernameSession(user);
